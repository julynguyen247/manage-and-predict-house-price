...
function App() {
  ...
  const handleSearch = () => {
    ...
    const propertyTypeString =
      Array.isArray(selectedPropertyTypes) && selectedPropertyTypes.length > 0
    ...
    const parsePair = (value) => {
      ...
      if (Array.isArray(value)) {
        ...
        const min = Number.isFinite(Number(minVal))
          ? Number(minVal)
          : undefined;
        const max = Number.isFinite(Number(maxVal))
          ? Number(maxVal)
          : undefined;
        return { min, max };
      }
      ...
      const max =
        !maxRaw || String(maxRaw).toLowerCase() === "infinity"
          ? undefined
  ...
  useEffect(() => {
    const fetchFeaturedProps = async () => {
      try {
        ...
        let res;
        try {
          res = await fetchFeaturedProperties({ featured: true, limit: 8 });
        } catch {
          res = await fetchFeaturedProperties({ featured: false, limit: 8 });
        }
        const data = res.data;
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        setNewsLoading(true);
        const res = await fetchFeaturedNews(3);
        const data = res.data;
        setFeaturedNews(data.results || data.data || []);
      } catch (error) {
  ...
  if (currentPage === "propertyList") {
    return (
      <PropertyList searchParams={searchParams} onBack={handleBackToSearch} />
    );
  }
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center justify-between text-sm text-gray-500">
                      ...
                      {news.author_name && <span>{news.author_name}</span>}
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {!loading && propertyListings.length === 0 && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                Không có bất động sản nổi bật
              </h3>
              <p className="text-gray-500 mb-4">
                Hiện tại chưa có bất động sản nào được đánh dấu nổi bật
              </p>
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-gray-400 mb-4">
                Nền tảng bất động sản hàng đầu Việt Nam, kết nối người mua và
