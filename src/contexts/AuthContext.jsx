...
export const AuthProvider = ({ children }) => {
  ...
  const validateToken = useCallback(async (storedToken) => {
    ...
    if (
      ...
      now - tokenValidationCache.lastChecked <
        tokenValidationCache.cacheDuration
    ) {
  ...
  const persistAuth = useCallback((authResponse) => {
    const accessToken =
      authResponse?.access || authResponse?.token || authResponse?.jwt;
    ...
    if (userPayload) {
      try {
        ...
        setUser(userPayload);
        setUser_fullname(
          userPayload.full_name || userPayload.first_name || null
        );
      } catch (e) {
  ...
  const googleLogin = useCallback(
    async (googleToken) => {
      try {
        ...
        const timeoutId = setTimeout(() => controller.abort(), 3000); // 15 giây timeout
        ...
        if (response.ok) {
          ...
          return {
            success: false,
            error: data.error || "Đăng nhập Google thất bại",
          };
        }
      } catch (error) {
